...
private:
    ...
    static constexpr uint32_t UNKNOWN_SIZE = 0xFFFF; // originall 0xFFFF
    ...
    static bool hasSg(dma_device_t *dma_hw)
    { // check Scatter/Gather mode
        return (dma_hw->s2mm_dmasr & DMASR_ISSG_MASK) || (dma_hw->mm2s_dmasr & DMASR_ISSG_MASK);
    }
};
