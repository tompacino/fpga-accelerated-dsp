...
AxiDMA::AxiDMA()
{
    auto result = run();
    if (result != DMA_OK)
        throw std::runtime_error("Can't initialize AXI DMA: " + std::to_string(result));
}
...
int AxiDMA::Transf(const AxiDmaBuffer *tx, AxiDmaBuffer *rx, size_t rx_len)
{
    try
    {
        ...
        if (txring->InitDescriptors(txring->GetBufferBaseAddr(), tx_len) != AxiDmaDescriptors::RING_OK)
        {
            delete txring;
            delete rxring;
            munmap(tx_data, tx_len);
        ...
        if (rxring->InitDescriptors(rxring->GetBufferBaseAddr(), rx_len) != AxiDmaDescriptors::RING_OK)
        {
            delete txring;
            delete rxring;
            munmap(tx_data, tx_len);
...
inline int AxiDMA::waitTxDirectComplete()
{
    ...
    while (!tx_irq && (timeout_10s > 0))
    { // search interrupt register
