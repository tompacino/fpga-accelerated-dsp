...
namespace LMS
{
  ...
  protected:
    ...
    T epsilon;
    T err;
    T pow; // May accumulate error with certain data types and ranges
  ...
  public:
    ...
    VSS(T stepSize, T alpha, T gamma, T epsilon, T minStep, T maxStep) : FSS<T, Taps, Normalised>(stepSize, epsilon), alpha(alpha), gamma(gamma), minStep(minStep), maxStep(maxStep), intStep(stepSize) {};
